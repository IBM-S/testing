...
expresion1 = r"lib_p\d+\.mdvrp"
expresion2 = r"lib_pr\d+\.mdvrp"

lista_expresiones = [expresion1, expresion2]
nombres = ["p", "pr"]
...
for num, expresion in enumerate(lista_expresiones):
    ...
    for instance_name in leer_Data:
        ...
        ### Estas features ya no sirven porque por cada cantidad de depots la instncias generaran ditinta cantidad de depots
        cv_dist_to_depot_i = []
        for i in range(0, len(model._depots)):
            ...
            cv_dist_to_depot_i.append((desv_standar_client_to_depot[i]/dist_prom_to_depot[i])*100 / max_possible_distance)
        ...
        features[f'avg_cv_dist_to_depots'] = np.mean(cv_dist_to_depot_i)
        ...
        ### nueva posible feature cantidad de avg clients per vehicle  per depot
        ...
        with open(csv_file, "a") as f:
            ...
            f.write(filtered_instances[contador].split(".")[0].split("_")[1])  # Usar nombre de instancia
