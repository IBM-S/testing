...
inst_dir = os.path.join(base_dir, "_inst_GOLDEN_dejar_corriendo")
...
# Extraer el número de la instancia desde el archivo
if [ -f "../../${dirInstances}/${instance}" ]; then
    dimension=$(grep "DIMENSION" "../../${dirInstances}/${instance}" | awk -F ':' '{print $2}' | tr -d '[:space:]')
    if [[ -n "$dimension" ]]; then
        new_instance_num=$((dimension - 1))
    else
        echo "Error: No se pudo encontrar la línea DIMENSION en el archivo de instancia."
        exit 1
    fi
else
    echo "Error: Archivo de instancia no encontrado en ../../${dirInstances}/${instance}."
    exit 1
fi
...
for filename in os.listdir(instances_dir):
    if filename.startswith("Golden_") and filename.endswith(".vrp"):
